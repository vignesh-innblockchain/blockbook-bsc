...
func fixBlockHash() {
	glog.Infof("fixBlockHash starting")
	diff := 0
	best, _, err := index.GetBestBlock()
	if err != nil {
		glog.Errorf("fixBlockHash GetBestBlock: %v", err)
		return
	}
	for height := uint32(1); height <= best; height++ {
		bi, err := index.GetBlockInfo(height)
		if err != nil {
			glog.Errorf("fixBlockHash db GetBlockInfo %d : %v", height, err)
			return
		}
		var retryCount int
		var hash string
		for {
			...
			if err != nil {
				retryCount++
				if retryCount > 5 {
					glog.Errorf("fixBlockHash chain GetBlockHash %d : %v", height, err)
					return
				}
		...
		if bi.Hash != hash {
			glog.Infof("fixBlockHash diff height %d, db %s, chain %s", height, bi.Hash, hash)
			diff++
		}
		if height%10000 == 0 {
			glog.Infof("fixBlockHash processing block %d", height)
		}
	}
	glog.Infof("fixBlockHash finished, found %d differences", diff)
}
